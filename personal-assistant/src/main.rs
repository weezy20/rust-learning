...
fn ask_name() {
    ...
    let name = read_line_with_prompt();
...
fn test_knowledge() {
    ...
    loop {
        let answer = read_line_with_prompt().parse::<i32>().unwrap();
        if answer == 2 {
            break;
        } else {
...
fn read_line_with_prompt() -> String {
    print!(">");
    stdout().flush().unwrap();
...
fn prove_count() {
    ...
    let num = read_line_with_prompt().parse::<i32>().unwrap();
    for i in 0..num {
        println!("{}!", i);
    }
}
